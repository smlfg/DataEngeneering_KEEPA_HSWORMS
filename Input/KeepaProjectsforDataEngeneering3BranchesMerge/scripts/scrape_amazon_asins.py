...
async def scrape_all(markets: list[str], dry_run: bool = False, max_pages: int = MAX_PAGES):
...
async def main():
    ...
    total_requests = sum(len(SEARCH_KEYWORDS[m]) for m in markets) * args.pages
    ...
    print(f"  Pages/keyword: {args.pages}")
    ...
    results, stats = await scrape_all(markets, dry_run=args.dry_run, max_pages=args.pages)
