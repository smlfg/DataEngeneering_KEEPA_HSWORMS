...
class PriceMonitorScheduler:
    ...
    async def run_scheduler(self):
        ...
        try:
            ...
            asyncio.create_task(self.price_consumer.consume())
            asyncio.create_task(self.deal_consumer.consume())
        ...
        # Launch 5-minute deal collector as background task
    ...
    def stop(self):
        """Stop the scheduler and cleanup connections"""
        self.running = False
        logger.info("🛑 Scheduler stopped")

        # Cleanup Kafka and Elasticsearch connections
        try:
            import asyncio

            loop = asyncio.get_event_loop()
            if loop.is_running():
                loop.create_task(price_producer.stop())
                loop.create_task(deal_producer.stop())
                if hasattr(self, "price_consumer"):
                    loop.create_task(self.price_consumer.stop())
                if hasattr(self, "deal_consumer"):
                    loop.create_task(self.deal_consumer.stop())
                loop.create_task(es_service.close())
        except Exception as e:
            logger.debug(f"Cleanup completed: {e}")
